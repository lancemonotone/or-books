...
function issueSidebarScrollRoot(root) {
  return root.querySelector("[data-issue-sidebar] .editor-issue-groups");
}

function focusIssueInSidebar(root, issueKey) {
  const item = root.querySelector(
    `[data-issue-item][data-issue-key="${CSS.escape(issueKey)}"]`,
  );
  if (!item) {
    return;
  }
  ...
  if (itemRect.top < scrollRect.top) {
    scrollRoot.scrollBy({
      top: itemRect.top - scrollRect.top,
      behavior: "smooth",
    });
  } else if (itemRect.bottom > scrollRect.bottom) {
    scrollRoot.scrollBy({
      top: itemRect.bottom - scrollRect.bottom,
      behavior: "smooth",
    });
  }
}
...
function bindIssueSidebarList(
  root,
  sidebarHost,
  { issues, audit, ui, onReorder, onNavigateToIssue },
) {
  bindIssueSidebarDnD(sidebarHost, { issues, audit, ui, root, onReorder });

  sidebarHost.querySelectorAll("[data-issue-key]").forEach((button) => {
...
function remountIssueSidebar(host, root, ctx) {
  ...
  host.innerHTML = renderIssueSidebar(
    ctx.issues,
    ctx.audit,
    ctx.ui,
    phaseFilter,
  );
  bindIssueSidebarList(root, host, ctx);
}
...
function bindIssueSidebarDnD(
  container,
  { issues, audit, ui, root, onReorder },
) {
  if (!container) {
    return;
  }
  ...
  const resolveDropTarget = (target) => {
    ...
    const zone =
      item?.closest("[data-phase-drop-zone]") ??
      target.closest("[data-phase-drop-zone]");
    if (!zone) {
      return null;
    }

    const targetPhaseId = Number(zone.dataset.phaseDropZone);
    if (!Number.isFinite(targetPhaseId)) {
      return null;
    }

    if (!allowCrossPhase()) {
      const singlePhase = Number(
        container.dataset.singlePhase || currentIssuePhaseFilter(ui, root),
      );
      if (targetPhaseId !== singlePhase) {
        return null;
      }
    }
    ...
    return {
      targetPhaseId,
      insertBeforeKey: item?.dataset.issueKey ?? null,
    };
  };
  ...
  container.addEventListener("dragleave", (event) => {
    const left = event.target.closest(
      "[data-issue-item], [data-phase-drop-zone]",
    );
...
export function renderAuditView(audit, root, onChange) {
  ...
  root
    .querySelector('[data-action="add-sprint"]')
    ?.addEventListener("click", () => {
      const nextId = sprints.length
        ? Math.max(...sprints.map((s) => Number(s.id) || 0)) + 1
        : 1;
...
function applyAuditForm(audit, root) {
  ...
  audit.sprints = [...root.querySelectorAll("[data-sprint-index]")].map(
    (card, index) => ({
      id: Number(
        card.querySelector(`[name="sprints.${index}.id"]`)?.value || index + 1,
      ),
      subtitle:
        card.querySelector(`[name="sprints.${index}.subtitle"]`)?.value ?? "",
      title: card.querySelector(`[name="sprints.${index}.title"]`)?.value ?? "",
      description:
        card.querySelector(`[name="sprints.${index}.description"]`)?.value ??
        "",
    }),
  );
}
...
export function renderIssuesView(
  ...
  const selected =
    issues.find((issue) => issue.key === ui.selectedIssueKey) ||
    issues[0] ||
    null;
  if (selected) {
  ...
  root.innerHTML = `
    <div class="editor-split">
      <aside class="editor-sidebar">
        ...
        <div data-issue-sidebar-host data-issue-sidebar${phaseFilter === "all" ? "" : ` data-single-phase="${escapeAttr(String(phaseFilter))}"`}>
  ...
  const onReorder = (dropIntent) => {
    ...
    if (sidebarHost) {
      ...
      const selectedIssue = issues.find(
        (issue) => issue.key === ui.selectedIssueKey,
      );
      if (selectedIssue) {
        remountIssueDetail(
          root,
  ...
  root
    .querySelector('[name="phase-filter"]')
  ...
  bindIssueSidebarList(root, sidebarHost, sidebarCtx);

  root
    .querySelector('[data-action="add-issue"]')
    ?.addEventListener("click", () => {
...
function bindIssueForm(
    ...
    el.addEventListener("change", () => {
      if (el.name === "sprint") {
        ...
        if (Number(issue.sprint) !== Number(previousPhase)) {
          const ordered = moveIssueToPhaseEnd(
            issues,
            audit,
            issue.key,
            issue.sprint,
  ...
  detail
    .querySelector('[data-action="add-acceptance"]')
    ?.addEventListener("click", () => {
      applyIssueForm(issue, root);
  ...
  detail
    .querySelectorAll('[data-action="remove-acceptance"]')
    .forEach((button) => {
      button.addEventListener("click", () => {
        applyIssueForm(issue, root);
  ...
  detail
    .querySelector('[data-action="add-evidence"]')
    ?.addEventListener("click", () => {
      applyIssueForm(issue, root);
      const linkedFiles = (issue.evidence || [])
        .map((item) => item.file)
        .filter(Boolean);
      openMediaFilePicker({
  ...
  detail
    .querySelectorAll('[data-action="remove-evidence"]')
    .forEach((button) => {
      button.addEventListener("click", (event) => {
        event.stopPropagation();
        applyIssueForm(issue, root);
...
function bindEvidenceForm(
  ...
  detail
    .querySelector('[data-action="delete-evidence-row"]')
    ?.addEventListener("click", () => {
      if (
        !window.confirm(
          `Remove ${row.file} from the gallery? The file stays in media/.`,
        )
      ) {
        return;
      }
    ...
    ?.addEventListener("click", () => {
      ...
      openIssueComposer({
        ...
        onCreate: (issue) => {
          ...
          onChange("issues");
          setEditorStatus(
            `Created issue ${issue.id} and linked this screenshot.`,
          );
          rerender();
        },
      });
...
function bindDecisionForm(
  ...
  detail
    .querySelectorAll('[data-action="add-option-evidence"]')
    .forEach((button) => {
      button.addEventListener("click", () => {
        applyDecisionForm(decision, root);
        const optionIndex = Number(button.dataset.optionIndex);
        const option = decision.options[optionIndex];
        const linkedFiles = (option.evidence || [])
          .map((item) => item.file)
          .filter(Boolean);
        openMediaFilePicker({
          blockedFiles: linkedFiles,
          callback: (file) => {
            decision.options[optionIndex].evidence =
              decision.options[optionIndex].evidence || [];
            if (
              decision.options[optionIndex].evidence.some(
                (item) => item.file === file,
              )
            ) {
              setEditorStatus(
                `${file} is already linked to this answer.`,
                true,
              );
              return;
            }
  ...
  detail
    .querySelectorAll('[data-action="pick-option-evidence"]')
    .forEach((button) => {
      button.addEventListener("click", () => {
        applyDecisionForm(decision, root);
        const optionIndex = Number(button.dataset.optionIndex);
  ...
  detail
    .querySelectorAll('[data-action="remove-option-evidence"]')
    .forEach((button) => {
      button.addEventListener("click", () => {
        applyDecisionForm(decision, root);
        const optionIndex = Number(button.dataset.optionIndex);
...
function applyDecisionForm(decision, root) {
  ...
  decision.recommendation =
    form.querySelector('[name="recommendation"]')?.value ?? "";
  decision.blocks = [...form.querySelectorAll('[name="blocks"]:checked')].map(
    (el) => el.value,
  );

  decision.options = [
    ...form.querySelectorAll("[data-option-index].editor-card"),
  ].map((card, optionIndex) => {
    const existing = decision.options?.[optionIndex] || {};
    const evidence = [
      ...card.querySelectorAll("[data-option-evidence-index]"),
    ].map((row, evidenceIndex) => ({
      file: existing.evidence?.[evidenceIndex]?.file || "",
      caption:
        row.querySelector(
          `[name="options.${optionIndex}.evidence.${evidenceIndex}.caption"]`,
        )?.value ?? "",
    }));
    return {
      value:
        card.querySelector(`[name="options.${optionIndex}.value"]`)?.value ??
        "",
      label:
        card.querySelector(`[name="options.${optionIndex}.label"]`)?.value ??
        "",
      description:
        card.querySelector(`[name="options.${optionIndex}.description"]`)
          ?.value ?? "",
      evidence,
    };
  });
